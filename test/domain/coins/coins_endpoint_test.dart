...
void main() {
  ...
  group('getBasicList method in', () {
    group('CoinsEndpoint test endpoint path creation', () {
      var sut = CoinsEndpoint(HttpRequestServiceMock(
          statusCode: 200, body: CoinListMockData.validResponseBody));
      ...
      test('with parameters', () async {
        ...
        expect(sut.endpointPath,
            "$apiVersionPath/coins/list?include_platform=true");
        await sut.getBasicList(includePlatform: false);
        expect(sut.endpointPath,
            "$apiVersionPath/coins/list?include_platform=false");
      });
    });
    ...
    group('CoinsEndpoint test endpoint response', () {
      test('with data in getting the correct response type', () async {
        ...
        expect(result.elementAt(0).platforms!["ethereum"],
            "0xb9ef770b6a5e12e45983c5d80545258aa38f3b78");
        expect(result.elementAt(0).platforms!["polygon-pos"],
            "0x8bb30e0e67b11b978a5040144c410e1ccddcba30");

        expect(result.elementAt(1).platforms!.length, 4);
        expect(result.elementAt(1).platforms!["ethereum"],
    ...
    group('CoinsEndpoint test for error handling', () {
      test('should throw an exception for failed request', () async {
        sut = CoinsEndpoint(HttpRequestServiceMock(
            statusCode: 500, body: CoinListMockData.validResponseBody));
        await expectLater(
    ...
    "error": "coin not found"
        ...
        await expectLater(sut!.getBasicList(includePlatform: true),
  ...
  group('getMarketList method in', () {
    ...
    group('CoinsEndpoint test endpoint response', () {
      ...
      test(
          'with data with complete parameters in getting the correct response type',
          () async {
        sut = CoinsEndpoint(HttpRequestServiceMock(
            statusCode: 200,
        ...
        expect(item1.image,
            "https://assets.coingecko.com/coins/images/17261/large/aAmmBptBALWETH.png?1626959531");
        ...
        expect(
            item1.lastUpdated, DateTime.tryParse("2023-03-23T09:08:01.270Z"));
    ...
    group('CoinsEndpoint test for error handling', () {
      test('should throw an exception for failed request', () async {
        sut = CoinsEndpoint(HttpRequestServiceMock(
            statusCode: 500, body: CoinMarketMockData.validResponseBody));
        await expectLater(sut!.getMarketList(vsCurrency: Currencies.php),
  ...
  group('getInfo method in', () {
    ...
    group('CoinsEndpoint test endpoint response', () {
      test('with data in getting the correct response type', () async {
        ...
        expect(result.detailPlatforms![''],
            <String, dynamic>{'decimal_place': null, 'contract_address': ''});
        ...
        expect(result.links!.homepage!.elementAt(1),
            "https://assets.coingecko.com/coins/images/1/thumb/bitcoin.png?1547033579");
        expect(result.links!.blockchainSite!.elementAt(3),
            "https://assets.coingecko.com/coins/images/1/thumb/bitcoin.png?1547033579");
        ...
        expect(
            List<String>.from(result.links!.reposUrl!['github']!).elementAt(2),
            "https://github.com/bitcoin/bips");
        expect(List<String>.from(result.links!.reposUrl!['bitbucket']!).isEmpty,
            true);
      ...
      test(
        ...
        expect(result.description!['sl'],
            "Bitcoin is the first successful internet money based on peer-to-peer technology; whereby no central bank or authority is involved in the transaction and production of the Bitcoin currency. It was created by an anonymous individual/group under the name, Satoshi Nakamoto. The source code is available publicly as an open source project, anybody can look at it and be part of the developmental process.\r\n\r\nBitcoin is changing the way we see money as we speak. The idea was to produce a means of exchange, independent of any central authority, that could be transferred electronically in a secure, verifiable and immutable way. It is a decentralized peer-to-peer internet currency making mobile payment easy, very low transaction fees, protects your identity, and it works anywhere all the time with no central authority and banks.\r\n\r\nBitcoin is designed to have only 21 million BTC ever created, thus making it a deflationary currency. Bitcoin uses the <a href=\"https://www.coingecko.com/en?hashing_algorithm=SHA-256\">SHA-256</a> hashing algorithm with an average transaction confirmation time of 10 minutes. Miners today are mining Bitcoin using ASIC chip dedicated to only mining Bitcoin, and the hash rate has shot up to peta hashes.\r\n\r\nBeing the first successful online cryptography currency, Bitcoin has inspired other alternative currencies such as <a href=\"https://www.coingecko.com/en/coins/litecoin\">Litecoin</a>, <a href=\"https://www.coingecko.com/en/coins/peercoin\">Peercoin</a>, <a href=\"https://www.coingecko.com/en/coins/primecoin\">Primecoin</a>, and so on.\r\n\r\nThe cryptocurrency then took off with the innovation of the turing-complete smart contract by <a href=\"https://www.coingecko.com/en/coins/ethereum\">Ethereum</a> which led to the development of other amazing projects such as <a href=\"https://www.coingecko.com/en/coins/eos\">EOS</a>, <a href=\"https://www.coingecko.com/en/coins/tron\">Tron</a>, and even crypto-collectibles such as <a href=\"https://www.coingecko.com/buzz/ethereum-still-king-dapps-cryptokitties-need-1-billion-on-eos\">CryptoKitties</a>.");
        expect(result.description!['ko'],
        ...
        expect(result.links!.homepage!.elementAt(1),
            "https://assets.coingecko.com/coins/images/1/thumb/bitcoin.png?1547033579");
        expect(result.links!.blockchainSite!.elementAt(8),
            "https://assets.coingecko.com/coins/images/1/thumb/bitcoin.png?1547033579");
        expect(result.links!.officialForumUrl!.elementAt(2), "");
        ...
        expect(result.links!.subredditUrl, "https://www.reddit.com/r/Bitcoin/");
        expect(
            List<String>.from(result.links!.reposUrl!['github']!).elementAt(1),
            "https://github.com/bitcoin/bips");
        expect(List<String>.from(result.links!.reposUrl!['bitbucket']!).isEmpty,
            true);
        expect(result.image!.thumb,
        ...
        expect(result.marketData!.currentPrice,
            {"aed": 101948, "link": 3820, "sats": 99954976});
        ...
        expect(result.marketData!.atlChangePercentage,
            {"aed": 16023.26026, "ars": 385180.1279, "sats": 5.12284});
        ...
        expect(result.marketData!.totalVolume,
            {"aed": 77994537033, "ars": 4359298184417, "sats": 76469824499968});
        ...
        expect(result.marketData!.priceChangePercentage1hInCurrency,
            {"aed": 0.55875, "ars": 0.55875, "link": 0.09927, "sats": 0.04844});
        ...
        expect(result.marketData!.lastUpdated,
            DateTime.parse("2023-03-25T02:43:27.743Z"));
        ...
        expect(result.developerdata, {
          ...
          "last_4_weeks_commit_activity_series": [0, 7]
        });

        expect(result.publicInterestStats,
            {"alexa_rank": 9440, "bing_matches": null});
        expect(result.statusUpdates!.length, 0);
        ...
        expect(result.tickers!.elementAt(0).timestamp,
            DateTime.parse("2023-03-25T02:30:36+00:00"));
        expect(result.tickers!.elementAt(0).lastTradedAt,
            DateTime.parse("2023-03-25T02:30:36+00:00"));
        expect(result.tickers!.elementAt(0).lastFetchAt,
            DateTime.parse("2023-03-25T02:30:36+00:00"));
        expect(result.tickers!.elementAt(0).isAnomaly, false);
        expect(result.tickers!.elementAt(0).isStale, false);
        expect(result.tickers!.elementAt(0).tradeUrl,
      ...
      test('should still return a result for error response', () async {
        sut = CoinsEndpoint(HttpRequestServiceMock(
            statusCode: 200, body: '''{"error": "coin not found"}'''));
    ...
    group('CoinsEndpoint test for error handling', () {
      test('should throw an exception for failed request', () async {
        sut = CoinsEndpoint(HttpRequestServiceMock(
            statusCode: 500, body: CoinInfoMockData.validResponseBody));
        await expectLater(sut!.getInfo(id: 'verus-coin'),
            throwsA(isA<NetworkRequestException>()));
      });
  ...
  group('getTickers method in', () {
    ...
    group('CoinsEndpoint test endpoint response', () {
      ...
      test(
        ...
        expect(result.tickers!.elementAt(0).convertedLast,
            {"btc": 0.99646975, "eth": 15.559817, "usd": 28307});
        expect(result.tickers!.elementAt(0).convertedVolume,
            {"btc": 5.847141, "eth": 91.303, "usd": 166099});
        expect(result.tickers!.elementAt(0).trustScore, "green");
        expect(result.tickers!.elementAt(0).bidAskSpreadPercentage, 0.200478);
        expect(result.tickers!.elementAt(0).timestamp,
            DateTime.parse("2023-03-29T10:27:01+00:00"));
        expect(result.tickers!.elementAt(0).lastTradedAt,
            DateTime.parse("2023-03-29T10:27:01+00:00"));
        expect(result.tickers!.elementAt(0).lastFetchAt,
            DateTime.parse("2023-03-29T10:27:01+00:00"));
        ...
        expect(result.tickers!.elementAt(0).isStale, false);
        expect(result.tickers!.elementAt(0).tradeUrl,
            "https://bittrex.com/Market/Index?MarketName=EUR-BTC");
        expect(result.tickers!.elementAt(0).tokenInfoUrl, null);
        expect(result.tickers!.elementAt(0).coinId, "bitcoin");
    ...
    group('CoinsEndpoint test for error handling', () {
      test('should throw an exception for failed request', () async {
        sut = CoinsEndpoint(HttpRequestServiceMock(
            statusCode: 500, body: CoinInfoMockData.validResponseBody));
        await expectLater(sut!.getTickers(id: 'verus-coin'),
            throwsA(isA<NetworkRequestException>()));
      });
  ...
  group('getHistory method in', () {
    ...
    group('CoinsEndpoint test endpoint path creation', () {
      ...
      test('with all parameters', () async {
        await sut.getHistory(
            id: 'bitcoin', date: DateTime(2022, 12, 30), localization: true);
    ...
    group('CoinsEndpoint test for error handling', () {
      test('should throw an exception for failed request', () async {
        sut = CoinsEndpoint(HttpRequestServiceMock(
            statusCode: 500, body: CoinHistoryMockData.validResponseBody));
        await expectLater(
            sut!.getHistory(id: 'bitcoin', date: DateTime(2022, 12, 30)),
            throwsA(isA<NetworkRequestException>()));
      });
  ...
  group('getMarketHistory method in', () {
    ...
    group('CoinsEndpoint test endpoint response', () {
      ...
      test(
          'with data in getting the correct response type with complete parameters',
          () async {
        sut = CoinsEndpoint(HttpRequestServiceMock(
            statusCode: 200,
    ...
    group('CoinsEndpoint test for error handling', () {
      test('should throw an exception for failed request', () async {
        sut = CoinsEndpoint(HttpRequestServiceMock(
            statusCode: 500, body: CoinMarketChartMockData.validResponseBody));
        await expectLater(
  ...
  group('getMarketHistoryWithDateRange method in', () {
    ...
    group('CoinsEndpoint test endpoint path creation', () {
      ...
      test('with parameters', () async {
        ...
        expect(sut.endpointPath,
            "$basePath?vs_currency=jpy&from=1392577232&to=1396587232");
      });
    });
